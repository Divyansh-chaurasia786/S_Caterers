...
<body>
  ...
  <main class="py-4 flex-grow-1">
    <div class="container">
      ...
      <div class="row g-2 g-md-3 mb-4">
        ...
        <div class="col-12 col-sm-4">
          <div class="stat-card">
            <div class="stat-icon-wrap" style="color: #28a745; background: rgba(40,167,69,0.1);"><i class="fa-solid fa-circle-check"></i></div>
            <div class="overflow-hidden">
              <div class="stat-val" style="color: #28a745;">100% Active</div>
              <div class="stat-lbl text-truncate">Storage Health</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row g-4">
        ...
        <div class="col-lg-4">
          <div class="admin-card">
            ...
            <form action="{{ route('admin.upload') }}" method="POST" enctype="multipart/form-data" id="upload-form" onsubmit="handleUploadSubmit(this)">
              @csrf
        ...
        <div class="col-lg-8">
          <div class="admin-card">
            ...
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3 pb-2 border-bottom">
              ...
              <!-- View Mode Pills -->
              <div class="nav admin-view-pills">
      ...
      <div class="modal-content modal-content-custom">
        <div class="modal-header modal-header-custom" style="background: linear-gradient(135deg, #6E1423, #4A0E17);">
          <h5 class="modal-title font-heading mb-0 text-white fw-bold" id="storageLimitLabel">
            <i class="fa-solid fa-triangle-exclamation text-warning me-2"></i> Upload Quota Limit Reached
          </h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body p-4 text-center">
          ...
          <h5 class="fw-bold mb-2" style="color: var(--admin-wine-dark);">Gallery Storage Limit Reached</h5>
          <p class="text-muted mb-0" style="font-size: 0.92rem; line-height: 1.6;">
            Your gallery has reached the maximum allowed items quota to preserve website speed and serverless health.
          </p>
          <div class="alert alert-warning border-0 mt-3 mb-0 text-start py-2 px-3" style="border-radius: 10px; font-size: 0.85rem;">
            <i class="fa-solid fa-circle-info me-1 text-warning"></i> <strong>How to fix:</strong> Please delete older photos or videos from the gallery list below before uploading new media assets.
          </div>
        </div>
  ...
  <script>
    ...
    // 5. Form Submit Loading Spinner State & Storage Limit Check
    const TOTAL_GALLERY_ITEMS_COUNT = {{ $images->count() }};
    const MAX_GALLERY_LIMIT = 65;

    function handleUploadSubmit(form) {
      if (TOTAL_GALLERY_ITEMS_COUNT >= MAX_GALLERY_LIMIT) {
      ...
      text.textContent = 'OPTIMIZING & UPLOADING...';
      return true;
    }
    ...
    function openImagePreviewModal(title, category, mediaPath, storageType, liveGalleryUrl, isVideo = false) {
      ...
      if (isVideo) {
        mediaContainer.innerHTML = `<video src="${mediaPath}" controls autoplay class="w-100" style="max-height: 440px; border-radius: 8px; box-shadow: 0 8px 30px rgba(0,0,0,0.5);"></video>`;
      } else {
